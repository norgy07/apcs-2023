...
public class Recur {
    ...
    public static boolean has(String input) {
        ...
        if (input.length() < 2)
            return false;
        /*
         * BELOW: returns whether character at index 0 is the same as 1 if it's or
         * recurses through has
         * assess whether the string is shorter than two characters or go back through
         * the previously
         * described conditions until either the charAt(0) and charAt(1) is "true" or
         * the length isn't
    ...
    public static boolean pot(String input) {
        ...
        /*
         * BELOW: return statement evaluates whether the first and second to last
         * characters are equal to one
         * another in addition to calling pot again with the first and last characters
    ...
    /*
     * method "two" has one argument that is an int "n" and if that is less than one
     * 1.0/2 is returned then 1.0
     * is divided by the sum of 2 and the method call of "two" of the number one
     * less than the current "n"
     */
    ...
    /*
     * "mop" is a method that returns 0 if the input is even and otherwise will
     * recurse through the mop again of
     * half of the previous value and add n mod 2
     * 11 is not divisible by 2 so goes to the second return statement and then 11/2
     * as an int is 5 so mop(5) +
     * 11%2 which is 1 SO: each method call is integer division of the preious term
     * by 2 and then returns the sum
    ...
    /*
     * "cat" will walk through the digits of a number and return the place values
     * and recurse with the number/10
     */
    ...
    // What does the following recursive method compute, if only positive params
    // passed?
    ...
    /*
     * This returns b^a because this will multiply the base number by itself the
     * amount
